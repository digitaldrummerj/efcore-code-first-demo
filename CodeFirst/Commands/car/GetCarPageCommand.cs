namespace CodeFirst.Commands
{
    ...
    public class GetCarPageCommand : IGetCarPageCommand
    {
        ...
        {
            var cars = this.carRepository.GetPage(pageOptions.Page.Value, pageOptions.Count.Value);
            if (cars == null)
            {
                return new NotFoundResult();
            }

            var (totalCount, totalPages) = this.carRepository.GetTotalPages(pageOptions.Count.Value);
